"""
Native-lib.cpp Template - Updated with AssetManager support
"""
...
class NativeLibTemplate:
    ...
    def generate(package_name):
        # Convert package name to JNI format with proper underscore encoding
        # JNI requires: _ → _1, then . → _
        # Example: com.example.rnbo_test → com_example_rnbo_1test
...
Java_{jni_package}_MainActivity_updateParam(JNIEnv *env,
                                                     jobject /* this */,
                                                     jstring name,
                                                     jfloat val) {{
    jboolean isCopy;
...
Java_{jni_package}_MainActivity_stringFromJNI(
        JNIEnv *env,
        jobject /* this */) {{
...
extern "C" JNIEXPORT void JNICALL
Java_{jni_package}_MainActivity_init(
        JNIEnv *env,
        jobject thiz,
...
// Used example for non-static function
    ...
    mspLoader->init(cDelcareParams, g_jvm, nativeAssetManager);
    mspLoader->start();
}}

extern "C" JNIEXPORT void JNICALL
Java_{jni_package}_MainActivity_destroy(
        JNIEnv *env,
        jobject /* this */) {{
